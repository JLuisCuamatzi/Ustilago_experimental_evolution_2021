...
###
###############################################################################################################################################
### File:           03.7_Plot_Coverage_ByQ_USMA_Pipeline.py
### Written by:     Jorge Luis Cuamatzi Flores
### Date:           2021_October_15
### Update:         
###
### Project:        This script writes a sge file to plot the coverage by quality
### Input:          A csv file with information and simple paths
### Output:         SGE to plot the coverage by quality
###
### How execute this script: python3 /mnt/Timina/lmorales/Public/Ustilago/C1/bin/scripts/03.7_Plot_Coverage_ByQ_USMA_Pipeline.py -d /mnt/Timina/lmorales/Public/Ustilago/C1/ -f /mnt/Timina/lmorales/Public/Ustilago/C1/ID.csv -t Mapping -M 8 -R /mnt/Timina/lmorales/Public/Ustilago/C1/bin/Rscript/Mapping/CoveragePlot.R
###############################################################################################################################################
## Libraries
import argparse
import os
...
def header(smpls_ID,sge):
    path_error = wd_project + "log/" + task + "/error/PlotCovByQual/"
    path_out = wd_project + "log/" + task + "/out/PlotCovByQual/"
...
##Error file
#$ -e''', path_error + smpls_ID + "." + fecha + ".PlotCovByQual.error",'''
##Out file
#$ -o''', path_out + smpls_ID + "." + fecha + ".PlotCovByQual.out",'''
#$ -S /bin/bash
## Job's name
#$ -N''', task + "_" + ID,'''
#$ -l vf='''+ memory +'''G
...
def mapping (smpls_ID):
    save_sge = wd_project + "bin/SGE/" + task + "/Coverage/ByQuality"
    if not os.path.exists(save_sge):    
        os.makedirs(save_sge)
    sge_name = save_sge + "/" + fecha + "_" + smpls_ID + "_PlotCovByQual.sge"
    sge = open(sge_name, "w")
    header(smpls_ID,sge)
    FILE1 = wd_project + "analysis/depth_coverage/" + smpls_ID + "_" + sample_name + ".Q20.depth.gz"
    FILE2 = wd_project + "analysis/depth_coverage/" + smpls_ID + "_" + sample_name + ".Q30.depth.gz"
    OUTPUT_PLOT1 = wd_project + "analysis/figures/Coverage/Q20/" + smpls_ID + "_Q20_CovPlot.png"
    OUTPUT_PLOT2 = wd_project + "analysis/figures/Coverage/Q30/" + smpls_ID + "_Q30_CovPlot.png"
    if not os.path.exists(wd_project + "analysis/figures/Coverage/Q20/"):
        os.makedirs(wd_project + "analysis/figures/Coverage/Q20/")
    if not os.path.exists(wd_project + "analysis/figures/Coverage/Q30/"):
        os.makedirs(wd_project + "analysis/figures/Coverage/Q30/")
    Rscript_out_path = wd_project + "log/Rscripts/" + task + "/PlotCovByQual/"
    if not os.path.exists(Rscript_out_path):
        os.makedirs(Rscript_out_path)
    Rscript_output = Rscript_out_path + smpls_ID + "_PlotCovByQual.Rout"
    ## Coverage requirements
    # Coverage requirements: paths
    #
    print ('''start=$(date +%s.%N)''', file = sge)
    ## Coverage by quality
    print ('''R CMD BATCH --no-save --no-restore "--args SIZE=200 STEP=200 SAMPLE=''' + "'" + smpls_ID + "' FILE1='" + FILE1 + "' FILE2='" + FILE2 + "' OUTPUT_PLOT1='" + OUTPUT_PLOT1 + "' OUTPUT_PLOT2='" + OUTPUT_PLOT2 + "'" + '" ' + Rscript_file + " " + Rscript_output, file = sge)
