...
###
###############################################################################################################################################
### File:           04.4_AddTsTvFiles_USMA_Pipeline.py
### Written by:     Jorge Luis Cuamatzi Flores
### Date:           2021_September_30
### Update:         
###
...
#python3 /mnt/Timina/lmorales/Public/Ustilago/C1/bin/scripts/04.4_AddTsTvFiles_USMA_Pipeline.py -d /mnt/Timina/lmorales/Public/Ustilago/C1/ -f /mnt/Timina/lmorales/Public/Ustilago/C1/ID.csv -t VCF -M 8
###############################################################################################################################################
## Libraries
import argparse
import os
...
##Error file
#$ -e''', path_error + ID + "_" + task + "_AddSampleTsTv.error",'''
##Out file
#$ -o''', path_out + ID + "_" + task + "_AddSampleTsTv.out",'''
#$ -S /bin/bash
## Job's name
#$ -N''', task + "_" + smpls_ID,'''
#$ -l vf='''+ memory +'''G
...
def mapping (smpls_ID):
    save_sge = wd_project + "bin/SGE/" + task + "/AddSampleTsTv"
    if not os.path.exists(save_sge):    
        os.makedirs(save_sge)
    sge_name = save_sge + "/" + fecha + "_" + smpls_ID + "_AddSampleTsTv.sge"
    sge = open(sge_name, "w")
    header(smpls_ID, sge)
    # paths
    # files
    TsTv_path = wd_project + "analysis/VCF/TsTv/PASS/"
    TsTv_file = smpls_ID + "_" + sample_name + ".TsTv.summary"
    print ('''start=$(date +%s.%N)''', file = sge)
    print ("## START: add a column with the sample name to the files with TsTv information", file = sge)
    print ("sed -i 's/$/\\t" + smpls_ID + "/' " + TsTv_path + TsTv_file, file = sge)
    print ("## END: add a column with the sample name to the files with TsTv information", file = sge)
