...
### Date:           2021_September_24
### Update:         2021_September_24
###
...
###
# How execute this script: python3 /mnt/Timina/lmorales/Public/Ustilago/C1/bin/scripts/04.2_CNV_variants_USMA_Pipeline.py -d /mnt/Timina/lmorales/Public/Ustilago/C1/ -f /mnt/Timina/lmorales/Public/Ustilago/C1/ID.csv -t CNV -R /mnt/Timina/lmorales/Public/Ustilago/C1/bin/Rscript/VCF/CNV_variants.R -M 8
###############################################################################################################################################
## Libraries
import argparse
import os
...
##Error file
#$ -e''', path_error + smpls_ID + "." + fecha + "." + tiempo + "_AddSample.error",'''
##Out file
#$ -o''', path_out + smpls_ID + "." + fecha + "." + tiempo + "_AddSample.out",'''
#$ -S /bin/bash
## Job's name
#$ -N''', task + "_" + ID,'''
#$ -l vf='''+ memory +'''G
...
def mapping (smpls_ID):
    save_sge = wd_project + "bin/SGE/" + task + "/AddSample"
    if not os.path.exists(save_sge):    
        os.makedirs(save_sge)
    sge_name = save_sge + "/" + fecha + "_" + smpls_ID + "_" + sample_name + "_AddSample.sge"
    sge = open(sge_name, "w")
    header(smpls_ID,sge)
    table_annotated = wd_project + "analysis/CNV/" + smpls_ID + "_" + sample_name + "_CNV.100.cnvnator"
    final_table_path = wd_project + "analysis/CNV/"
    if not os.path.exists(final_table_path):
        os.makedirs(final_table_path)
    final_table = final_table_path + smpls_ID + "_CNV.txt"
    ...
    Rscript_output = Rscript_out_path + smpls_ID + "_AddSample.Rout"
